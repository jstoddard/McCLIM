...
;;; intersection tests
(let ((el1 (make-ellipse* 200 200 100 0 0 50))
      (el2 (make-ellipse* 200 200 100 0 0 50 :start-angle 0 :end-angle (/ pi 2)))
      (el3 (make-ellipse* 200 200 100 0 0 50 :start-angle (/ pi 4) :end-angle (* 3 (/ pi 4))))
      (el4 (make-ellipse* 200 200 100 0 0 50 :start-angle (* 3 (/ pi 4)) :end-angle (/ pi 4)))
      (l1 (make-line* 0 200 1000 200))
      (l2 (make-line* 0 175 1000 175))
      (l3 (make-line* 150 200 250 200)))
  (assert (region-equal (make-line* 100 200 300 200) (region-intersection l1 el1)))
  (assert (linep (region-intersection l2 el1)))
  (assert (region-equal l3 (region-intersection l3 el1)))

  (assert (region-equal (make-line* 200 200 300 200) (region-intersection l1 el2)))
  (assert (linep (region-intersection l2 el2)))
  (assert (region-equal (make-line* 200 200 300 200) (region-intersection l3 el2)))

  (assert (region-equal +nowhere+ (region-intersection l1 el3)))
  (assert (let ((set (region-intersection l2 el3)))
            (and (region-set-p set)
                 (every #'linep (region-set-regions set)))))
  (assert (region-equal +nowhere+ (region-intersection l3 el3)))

  (assert (region-equal (make-line* 100 200 300 200) (region-intersection l1 el4)))
  (assert (let ((line (region-intersection l2 el4)))
            (and (linep line)
                 (null (region-equal line l2)))))
  (assert (region-equal l3 (region-intersection l3 el4))))
